 ...
 * 该类型的构造过程应提供良好的方式或较为简单的使用方法，或可如{@link DBC}类型一样，将构造行为委托至
 * {@link DBCFactory}<br>
 * 20240730追加：当前该有实现类线程安全的连接池实现该，具体可见
 * {@link io.github.baifangkual.jlib.db.impl.pool.ConnectionPoolProxyDataSource}<br>
 ...
 *     }
 * </pre>
 * @see DBC#execQuery(String, ResultSetRowMapping)
 * @see DBC#execQuery(ResultSetMapping, String)
 * @see #close()
 * @see ResultSetMapping
 * @see ResultSetRowMapping
 */
public interface DBCPool extends DBC, Closeable {

    /**
     * 获取一个连接对象，不同的{@link DBCPool}的该方法的行为应该是不同的
     *
