...
/**
 * <b>ID Generator</b><br>
 * 线程安全，生成的ID为雪花ID，共 63 位有效位，符号位永远为 0<br>
 ...
 * 该Id构成: {@code 0(符号位) - 时间偏移量 - 数据中心标识 - 机器码 - 同毫秒序列号}<br>
 * 该工具类对时间回溯没有太大抗性，当两次通过 {@link #nextId()} 获取id的间隔中发生多次
 * 或单次大于 {@value #MAX_FAULT_TOLERANT_BACKTRACKING_CAPACITY} 毫秒的时间回溯时，将抛出运行时异常<br>
 * <p>
 * 该算法及代码参考：
...
public final class Idg {
    ...
    /**
     * 宇宙热寂时间<br>
     * {@value EPOCH_BEGIN} + 2199023255551L = 2090-09-07 23:47:35(UTC/GMT+08:00)<br>
     * 不能在 {@code System.currentTimeMillis()} 大于该值时使用
     */
    static final long EPOCH_END = EPOCH_BEGIN + 2199023255551L;
    /**
     ...
     */
    public Idg(long machineId, long centerId) {
        long nowTimeMillis = nowSystemTime();
    ...
    public synchronized long nextId() {
        ...
        lastTime = timestamp;
        return buildId(timestamp, centerId, machineId, sequence);
    }

    private static long buildId(long timeMillis, long centerId, long machineId, long sequence) {
        // 时间戳部分 | 数据中心部分 | 机器标识部分 | 序列号部分
        return ((timeMillis - EPOCH_BEGIN) << TIMESTAMP_OFFSET)
     ...
     * @param sequence      序列号
     */
    public record Id(long genTimeMillis, long centerId, long machineId, long sequence) {
        /**
        ...
        public static Id ofLongId(long snowflakeId) {
            ...
            long centerId = (~(-1 << CENTER_ID_BITS)) & (snowflakeId >> CENTER_ID_OFFSET);
            long machineId = (~(-1 << MACHINE_ID_BITS)) & (snowflakeId >> SEQUENCE_BITS);
            long sequence = (~(-1 << SEQUENCE_BITS)) & snowflakeId;
            return new Id(genTimeMillis, centerId, machineId, sequence);
        }

        public long toLongId() {
            return buildId(genTimeMillis, centerId, machineId, sequence);
        }
    }
}
